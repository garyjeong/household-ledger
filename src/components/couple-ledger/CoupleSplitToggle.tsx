...
export function CoupleSplitToggle({
  ...
  // 분할 비율 변경 핸들러
  const handleSplitChange = useCallback((newSplit: SplitRule) => {
  ...
  // 직접 입력 핸들러
  const handleDirectInput = useCallback((field: 'me' | 'partner', value: string) => {
  ...
  // 프리셋 적용
  const handlePreset = useCallback((preset: { me: number; partner: number }) => {

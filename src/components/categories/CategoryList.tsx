...
export function CategoryList({
  ...
  // 기본/커스텀 그룹핑
  const { default: defaultCategories, custom: customCategories } =
    groupCategoriesByDefault(filteredCategories)
        ...
        <CardContent className='p-4'>
          <div className='flex items-center gap-3'>
            ...
            <div className='flex-1 min-w-0'>
              ...
              <p className='text-sm text-gray-500'>{transactionTypeLabels[category.type]}</p>
            </div>
    ...
    return (
      <div className='text-center py-12'>
        ...
        <h3 className='text-lg font-medium text-gray-900 mb-2'>
          {selectedType
            ? `${transactionTypeLabels[selectedType]} 카테고리가 없습니다`
            : '카테고리가 없습니다'}
        </h3>
      ...
      {customCategories.length > 0 && (
        <div>
          <div className='flex items-center gap-2 mb-3'>
            ...
            <Badge variant='secondary'>{customCategories.length}개</Badge>
          </div>
          <div className='space-y-2'>
            {customCategories.map(category => renderCategoryCard(category, false))}
          </div>
        </div>
      )}
    </div>

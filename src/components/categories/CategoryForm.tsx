...
export function CategoryForm({
  ...
  return (
    <Card className='w-full max-w-md'>
      <CardHeader>
        <CardTitle>{mode === 'create' ? '새 카테고리 추가' : '카테고리 정보 수정'}</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onFormSubmit)} className='space-y-4'>
          ...
          {/* 색상 선택 */}
          <div className='space-y-2'>
            <ColorPicker label='카테고리 색상' value={selectedColor} onChange={handleColorChange} />
          ...
          {selectedColor && (
            <div className='space-y-2'>
              ...
              <div className='flex items-center gap-2 p-3 rounded-lg border border-gray-200'>
                ...
                <span className='font-medium'>{watch('name') || '카테고리명'}</span>

...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-10 w-full items-center justify-between rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 cursor-pointer',
      // 부드러운 상태 변화 애니메이션 추가
      'transition-all duration-200 ease-in-out',
      'hover:border-slate-400 hover:bg-slate-50/50',
      'focus:outline-none focus:ring-2 focus:ring-slate-300/30 focus:border-slate-400',
      'data-[state=open]:ring-2 data-[state=open]:ring-slate-300/30 data-[state=open]:border-slate-400',
  ...
  >
    <span className='absolute left-2 flex h-3.5 w-3.5 items-center justify-center'>
      <SelectPrimitive.ItemIndicator>
        <Check className='h-4 w-4 transition-opacity duration-150' />
      </SelectPrimitive.ItemIndicator>
    </span>

...
export function InboxList({ className = '', showGrouping = true, pageSize = 20 }: InboxListProps) {
    ...
    <div className={className}>
      ...
      {!isMobile && (
        <div className='bg-surface-page rounded-lg p-3 mb-4 text-xs text-text-600'>
          <div className='flex items-center gap-4'>
            <span>
              <kbd className='px-1 py-0.5 bg-white rounded'>↑↓</kbd> 선택
            </span>
            <span>
              <kbd className='px-1 py-0.5 bg-white rounded'>E</kbd> 편집
            </span>
            <span>
              <kbd className='px-1 py-0.5 bg-white rounded'>Del</kbd> 삭제
            </span>
            <span>
              <kbd className='px-1 py-0.5 bg-white rounded'>Esc</kbd> 취소
            </span>
          </div>
        </div>
      )}
          ...
          <div key={date.toISOString()}>
            ...
            {showGrouping && (
              <div className='flex items-center gap-3 mb-3'>
                <h3 className='text-sm font-medium text-text-700'>{getDateLabel(date)}</h3>
                <div className='flex-1 h-px bg-stroke-200' />
                <div className='text-xs text-text-500'>{dayTransactions.length}건</div>
              </div>
            )}
      ...
      {totalPages > 1 && (
        <div className='flex items-center justify-between mt-8'>
          <div className='text-sm text-text-600'>
            ...
            {Math.min(currentPage * pageSize, transactions.length)} / {transactions.length}
          </div>
          ...
          <div className='flex items-start justify-between'>
            <div className='flex-1 min-w-0'>
              ...
              <div className='text-sm text-text-600 space-y-1'>
                ...
                {transaction.memo && (
                  <div className='text-text-500 text-xs truncate'>{transaction.memo}</div>
                )}
              </div>
            </div>
  ...
  return (
    <div
      className={`flex items-center gap-4 p-3 rounded-lg border transition-colors cursor-pointer ${
        isSelected
          ? 'ring-2 ring-brand-500 bg-brand-50 border-brand-200'
          : 'border-stroke-200 hover:bg-surface-page'
      } ${transaction.isDuplicate ? 'border-orange-200 bg-orange-50' : ''}`}
      onClick={onSelect}
    >
      {/* Type indicator */}
      <div className={`p-2 rounded-full ${typeColorClass.split(' ')[2]}`}>{typeIcon}</div>

      {/* Main content */}
      <div className='flex-1 grid grid-cols-5 gap-4 items-center min-w-0'>
        {/* Date */}
        <div className='text-sm text-text-600'>{format(transaction.date, 'MM/dd')}</div>

        {/* Amount */}
        <div className='text-right'>
          <div className='font-semibold text-text-900'>{formatAmount(transaction.amount)}</div>
          <div className={`text-xs ${typeColorClass.split(' ')[0]}`}>{typeLabel}</div>
        </div>
        ...
        <div className='flex items-center gap-2 min-w-0'>
          ...
          <span className='text-sm text-text-700 truncate'>{transaction.category.name}</span>
        </div>

        {/* Account */}
        <div className='text-sm text-text-600 truncate'>{transaction.account.name}</div>

        {/* Memo */}
        <div className='text-sm text-text-500 truncate'>{transaction.memo || '-'}</div>
      </div>

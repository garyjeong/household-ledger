...
**주요 변경사항**: Enterprise급 보안 강화, N+1 쿼리 최적화, 커서 페이지네이션
...
- **🏠 그룹 관리**: 초대 코드를 통한 그룹 생성/참여 (직접 연결 방식)
- **💰 Account-less 가계부**: 계좌 없이 거래만으로 실시간 잔액 계산
- **📊 실시간 대시보드**: 동적 월별 요약, 카테고리 분석, 지출 트렌드
- **📱 반응형 UI**: 모바일 퍼스트 디자인, 5개 핵심 페이지
- **⚡ 단순화된 아키텍처**: 9개 핵심 테이블로 성능 최적화
